                ...
                for update in updates["result"]:
                    ...
                    if "callback_query" in update:
                        ...
                        if data == "bypass_menu":
                            ...
                            edit_message(chat_id, message_id, "🎮 قسم السكريبتات والهاكات قيد التحديث...")
                            answer_callback(cq_id)
                        elif data == "supported_links":
                            edit_message(chat_id, message_id, "🌐 الروابط المدعومة حالياً:\n- LootLabs\n- Boosty\n- Linkvertise")
                            answer_callback(cq_id)
                        elif data == "bot_help":
                            edit_message(chat_id, message_id, "📖 بوت متخصص في تخطي الروابط المختصرة واستخراج الروابط المباشرة.")
                            answer_callback(cq_id)
                        elif data == "stats":
                            answer_callback(cq_id, "تم إنجاز آلاف الطلبات بنجاح!")
                        ...
                        if user_text == "/start":
                            ...
                            try:
                                ...
                                # محاولة فحص التوجيه المباشر أولاً (Redirects) لمواقع مثل Boosty
                                bypassed_link = None
                                try:
                                    r = requests.get(target_url, allow_redirects=True, timeout=10, headers={"User-Agent": "Mozilla/5.0"})
                                    if r.url != target_url and "html" not in r.text[:100].lower():
                                        bypassed_link = r.url
                                except:
                                    pass
                                
                                # إذا لم يتم التوجيه المباشر، نلجأ لفحص الـ API البديل
                                if not bypassed_link:
                                    api_url = f"https://bypass.city/api?url={target_url}"
                                ...
                                if bypassed_link and "http" in str(bypassed_link) and "<html" not in str(bypassed_link).lower():
                                    result_text = (
                                    ...
                                    requests.post(URL + "editMessageText", json={"chat_id": chat_id, "message_id": msg_id, "text": "❌ فشل التجاوز، هذا الرابط يتطلب تفاعل متصفح حقيقي (Cloudflare/Captcha)."})
