...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "http://" in user_text or "https://" in user_text:
        ...
        if extracted_result and "unsupported" not in res_lower and "error" not in res_lower and "traceback" not in res_lower:
            ...
            fail_message = "❌ فشل في تجاوز الرابط! الرابط غير صالح أو منتهي الصلاحية."
            await update.message.reply_text(fail_message, reply_markup=get_main_keyboard())
    else:
...
def main():
    ...
    print("Bot is running without inline buttons...")
    app.run_polling()
